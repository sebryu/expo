...
>
> **Spike result (2026-06-10): GO.** H1 and H2 demonstrated end-to-end in `~/Work/module-host-spike/` (host shell + module workspace + JS protocol client + local spec server), including on a **Release-configuration host binary** (embedded JS, no Metro/dev tooling): EAS OTA load 276–868 ms cold / 271 ms warm-cache / 63 ms offline-fallback, eval 7–12 ms (2 ms after trimming the export to 7.4 KB), two OTA version pickups with zero host rebuilds. See `module-host-spike/GO_NO_GO_MEMO.md` for the scorecard and the eight implementation findings the plan didn't anticipate — the original five (EAS CDN asset auth via `extensions.assetRequestHeaders`; `runtimeVersion` charset; Metro guarded-runtime collision → `transformer.globalPrefix`; eval global snapshot/restore; raw-CJS host module exports) plus week 4's three (release builds need the legacy FS API to read bundled assets; Metro patch drift leaves the export byte-identical; the streams-polyfill trim is implemented as a post-export script). Residual caveats, not mechanisms: TestFlight upload itself, physical mid-tier device timings.
>
...
- `resolver.resolveRequest` rewrites externals (`react`, `react-native`, `expo-*`, `@acme/module-sdk`) to a shim that reads `global.__HOST_MODULES__[name]` — Metro has no native "externals", but a custom resolver redirect to a generated shim file is a standard, working pattern. This keeps module bundles small (tens of KB) and guarantees one React instance. ✅ spike-verified, with three mandatory details: `transformer.globalPrefix` on the module build (Metro's guarded runtime install otherwise collides with the host registry and the bundle silently runs a host module), host must export **raw CJS** module objects (`require('react')`, not an ESM namespace — Babel wildcard interop throws on a namespace's `default`), and the loader must snapshot/restore Metro globals (incl. the prefixed ones) around eval.
...
`runtimeVersion` of every module update is set to `host-sdk-2` — ⚠️ **corrected by spike:** strings are NOT arbitrary; EAS enforces alphanumeric plus `_ + . ( ) -` (no `@`). Matching is exact-string and server-side filtered (✅ verified live), so expo-updates' own selection mechanism refuses modules built for an incompatible host (Section 8).
...
5. expo-router cannot register routes at runtime (routes come from the filesystem `require.context` at build time), so the host ships one catch-all `app/mod/[...slug].tsx` that maps the slug to a module route and renders it inside `<ModuleErrorBoundary>` + a render watchdog; tab/drawer entries are generated from composition manifests. Unload = unmount + delete registry entry + (best-effort) drop SDK handle. ✅ spike-verified (catch-all mount, module-internal nav in URL space, unload/replace incl. live version pickup; watchdog still deferred).
6. ⚠️ **Added by spike:** asset downloads from `assets.eascdn.net` require per-asset authorization delivered in the protocol's `extensions` multipart part (`assetRequestHeaders[assetKey].authorization = "EAS-HMAC-SHA256 …"`); a client that ignores `extensions` gets 403 on the bundle download.
...
- **Host Preview app**: an **expo-dev-client build of the real host shell** (same binary as production plus dev-launcher), published internally via EAS Build. In dev mode its ModuleLoader accepts `http://localhost:8082/index.bundle?platform=ios&dev=false&minify=false` — the agent runs Metro in the workspace and the preview app hot-loads the module through the *same* loader/SDK path as production. ✅ spike-verified: edit→render loop ~2–5 s (re-fetch 170 ms incl. incremental rebuild); requesting `dev=false` from the workspace Metro sidesteps HMR-client entanglement. This mirrors how expo-dev-client + `expo-updates-interface` already let dev-launcher drive update loading (✅ pattern verified in `ExpoDevLauncherReactDelegateHandler.swift`).
...
**EAS Update *is* the registry.** One EAS project ("modules"), one branch per module×channel (`module/chat/production`), updates as versions. At the agreed scale (one app, 10–50 modules → ≤150 branches across three channels) this stays manageable in the EAS dashboard. It buys, with zero code: artifact storage + global CDN, the open expo-updates protocol (`https://u.expo.dev/{projectId}` with `expo-channel-name`/`expo-runtime-version` headers — ✅ header set verified against `FileDownloader`), code-signing signature delivery, staged rollout (`eas update --rollout-percentage`, `eas channel:rollout`), instant rollback (`eas update:republish` an older group), and an audit trail in the EAS dashboard. ✅ The core "fetch a branch's update from app code without launching it" loop is spike-proven against live `u.expo.dev` (cold load 338 ms; offline cache relaunch; in-process version pickup after a new publish). Publish plumbing that works: `expo export --platform ios --no-bytecode` + `eas update --channel <name> --skip-bundler --input-dir dist --environment preview` (`--environment` is mandatory in non-interactive mode; the CLI auto-installs `expo-updates` into the workspace).
...
|---|---|---|
| Plain-JS export path blocked or fragile on EAS (default pipeline is Hermes bytecode, which cannot be eval'd) | Kills the JS-loader approach | **✅ Resolved by spike:** `expo export --no-bytecode` + `eas update --skip-bundler --input-dir dist` published and served plain JS end-to-end; published bundle fetched, hash-verified, and eval'd in-app (incl. release build). Residual resolved in week 4: the ~63 KB `@expo/cli`-appended streams polyfill is dropped by a post-export trim script (70 KB → 7.4 KB, eval 2 ms, published + verified OTA) |
...
| Hermes must parse plain JS at runtime (no HBC eval) | Slower module mount on low-end devices | Small bundles via externals; eval once per process, cache exports; lazy-mount on first navigation; ✅ spike measured parse cost: 7–16 ms for the 70 KB bundle, 2 ms trimmed (release build, simulator; physical mid-tier device still unmeasured); later: sidecar adds ahead-of-time HBC compile on download |
...
| Metro externals via custom `resolveRequest` is unofficial | Build breakage on Metro upgrades | Metro version pinned by Expo SDK choice; ✅ spike exercised a Metro patch split (workspace metro family 0.84.3 vs host 0.84.4): export byte-identical modulo the version string, loaded OTA by the unchanged host; golden-file tests on bundle output in module template CI |
